...
$$
Novelty = 1 - \min(LINK \, of\, all\, citations)
$$
...
$$
Novelty_p = 1 - \min_{c \in citations(p)} LINK(t_p, i_p, j_c)
$$
...
| `min_link` | float | 最小 LINK 值（0-1） |
| `novelty` | float | 新颖性指标（0-1） |
| `mean_link` | float | 平均 LINK 值 |
| `max_link` | float | 最大 LINK 值 |
| `radical_90` | int | 是否为前 10% 高新颖专利 |
